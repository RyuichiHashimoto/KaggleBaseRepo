...
ARG_Parameter: TypeAlias = Tuple[Union[UnitRootParameter, Dict[str, Any], Union[SteadyParameter, Dict[str, Any]]]]


class UnitRoot2SteadyDataset(Dataset):
    def __init__(self, parameters: Iterable[ARG_Parameter]):
        super().__init__()
        assert len(parameters) == 2
        self.parameters = [UnitRootParameter.from_dict(parameters[0]), SteadyParameter.from_dict(parameters[1])]
    ...
    def _create_dataset(self) -> None:
        ...
        param2: SteadyParameter = self.parameters[1]
        data = np.random.normal(loc=param2.mean, scale=np.sqrt(param2.var), size=param2.size)
